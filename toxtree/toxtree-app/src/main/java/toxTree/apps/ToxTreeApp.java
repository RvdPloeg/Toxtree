...
public class ToxTreeApp extends CompoundMethodApplication {
	 ...
	 */
	public ToxTreeApp() {
		super("Estimation of Toxic Hazard - a Decision Tree Approach ",
	...
	protected DataModule createDataModule() {
		...
		if (methods.size() == 0) {
			logger.log(Level.WARNING,
					"No decision trees found! Have you installed the application correctly?");
	...
	public static void main(String[] args) {
		...
		try {
			options.parse(args);
			if (options.isNoui()) {
				ToxtreeHeadless app = new ToxtreeHeadless(options);
				app.run();
			} else
	...
	protected ToxtreeOptions parseCmdArgs(String[] args) throws Exception {
		...
		file4Batch = new File(option.getBatch());
		if (!file4Batch.exists()) {

			logger.severe("File do not exists!\t"
					+ file4Batch.getAbsolutePath());
			file4Batch = null;
		}
		return option;
	}
