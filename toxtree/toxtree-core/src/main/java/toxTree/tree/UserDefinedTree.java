 ...
 * {@link toxTree.core.IDecisionMethod} interface. It stores categories in a
 * {@link toxTree.core.IDecisionCategories} and the tree itself in a
 * {@link toxTree.tree.DecisionNodesList}. The tree is editable, i.e. rules can
 * be added , removed and modified.
 * 
 * @author Nina Jeliazkova <b>Modified</b> 2005-11-10
 */
public class UserDefinedTree extends AbstractTree implements
		IDecisionInteractive, IMolecularDescriptor,
		IObject2Properties<UserDefinedTree> {
	 ...
	 */
	public UserDefinedTree(IDecisionCategories classes, String[] customRules,
			int[][] customTransitions, IDecisionNodesFactory nodesFactory)
			throws DecisionMethodException {
		super(classes);
		setNodesFactory(nodesFactory);
		treeRoot = 0;
		this.rules = nodesFactory.createNodesList(getCategories(), customRules,
				customTransitions);
		if (rules instanceof Observable)
			((Observable) rules).addObserver(this);
		if (categories instanceof Observable)
		...
		if (rules instanceof DecisionNodesList)
			for (int i = 0; i < customTransitions.length; i++) {
				...
				if (customTransitions[i][0] != 0)
					node.setBranch(false, ((DecisionNodesList) rules)
							.getNode(customTransitions[i][0] - 1));
				if (customTransitions[i][1] != 0)
					node.setBranch(true, ((DecisionNodesList) rules)
							.getNode(customTransitions[i][1] - 1));
	...
	public IDecisionRuleList hasUnreachableRules() {
		...
		/*
		 * 
		 * DecisionNodesList nodes = (DecisionNodesList) rules; for (int i=0; i
		 * < nodes.size();i++) nodes.getNode(i).setVisited(false); DecisionNode
		 * rule = nodes.getNode(treeRoot);
		 * 
		 * visitRules(rule); for (int i=0; i < nodes.size();i++) if
		 * (!nodes.getNode(i).isVisited()) { if (unvisited == null) unvisited =
		 * nodesFactory.createNodesList(); unvisited.addNode(nodes.getNode(i));
		 * } return unvisited;
		 */
	...
	protected void visitCategories(DecisionNode rule,
		...
		for (int i = 0; i < b.length; i++) {
			...
			if (c != null)
				usedCategories.addCategory(c);
		}

		DecisionNode node = (DecisionNode) rule.getBranch(true);
		if (node != null)
			visitCategories(node, usedCategories);
		node = (DecisionNode) rule.getBranch(false);
		if (node != null)
			visitCategories(node, usedCategories);
	}
	...
	public IDecisionCategories hasUnusedCategories() {
		...
		if (getCategories().size() == usedCategories.size())
			return null;
	 ...
	 * 
	 * @see
	 * toxTree.tree.AbstractTree#verifyRules(org.openscience.cdk.AtomContainer,
	 * toxTree.core.IDecisionResult, int)
	 */
	@Override
	protected boolean verifyRules(IAtomContainer mol, IDecisionResult result,
		...
		if (rule == null)
			throw new DecisionMethodException(
					"Top rule not defined! Possible reason: Empty decision tree!");
		...
		while (classID == 0) {
			...
			try {
				...
				MolFlags mf = (MolFlags) molToAnalyze
				...
				mf.setResidues(null); // clear that ; the next rule will set up
										// them if necessary
			...
			if (category_on_error != null)
				nextCategory = category_on_error;
			else
				nextCategory = getCategory(rule, answer);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see toxTree.core.IDecisionMethod#getBranch(toxTree.core.IDecisionRule,
	 * boolean)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see toxTree.core.IDecisionMethod#getCategory(toxTree.core.IDecisionRule,
	 * boolean)
	 */
	...
	public void setParameters(IAtomContainer mol) {
		if (getInteractive()) {
			...
			if (c != null)
				JOptionPane.showMessageDialog(null, c, getTitle(),
	...
	private void readObject(ObjectInputStream in) throws IOException,
		...
		for (int i = 0; i < rules.size(); i++) {
			...
			if ((c != null) && (categories.indexOf(c) == -1))
				categories.add(c);
	...
	protected void setArrayValue(ArrayResult result, int index,
			IAtomContainer mol, String propertyName) {
	...
	public DescriptorSpecification getSpecification() {
		return new DescriptorSpecification("http://toxtree.sourceforge.net",
				getTitle(), this.getClass().getName(), "Toxtree plugin");
	}
	...
	public void done() throws DecisionMethodIOException {
		...
		for (int i = 0; i < nodes.size(); i++) {
			...
			if (!nodes.getNode(i).isVisited()) {
				if (unvisited == null)
					unvisited = new DecisionNodesList();
