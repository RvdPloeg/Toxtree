 ...
 * base class for all rules in {@link toxTree.apps.ToxTreeApp} application.
 * 
...
public abstract class AbstractRule extends Observable implements IDecisionRule,
     ...
     * 
     */
	private static final long serialVersionUID = 7104944189094581855L;
	...
	public void setTitle(String name) {
		// if (changes != null)
		// changes.firePropertyChange("Name",this.title,title);
		this.title = name;
		setChanged();
		notifyObservers();
	}
	...
	public String toString() {
		...
		b.append(title);
		if (!isImplemented())
			b.append("\t(NOT IMPLEMENTED, ASSUMING FALSE!)");
		return b.toString();
	}
	...
	public static IDecisionRule createRule(String className)
		 ...
		 * classDefinition.newInstance(); } catch (Exception e) { object = null;
		 * throw new DecisionMethodException(e); } return object;
		 */
		try {
	...
	public void setExampleMolecule(IAtomContainer mol, boolean ruleResult) {
		...
		notifyObservers();
		/*
		 * } catch (CDKException x) { examples[index] = ""; }
		 */
	}
	...
	public IAtomContainer getExampleMolecule(boolean ruleResult)
		...
		try {
			...
			if (ruleResult)
				mol.setID("'YES example'");
			else
				mol.setID("'NO example'");
			return mol;
		} catch (Exception x) {
			throw new DecisionMethodException(x);
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see toxTree.core.IDecisionRule#addPropertyChangeListener(java.beans.
	 ...
	 * (non-Javadoc)
	 * 
	 * @see toxTree.core.IDecisionRule#removePropertyChangeListener(java.beans.
	 * PropertyChangeListener)
	 */
