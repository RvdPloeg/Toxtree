...
public class RuleCarbohydrate extends RuleSubstructures {
	...
	protected ArrayList<String> idsCyclic;
	protected ArrayList<String> idsAcyclic;
	...
	public RuleCarbohydrate() throws Exception {
		super();

		idsCyclic = new ArrayList<String>();
		idsAcyclic = new ArrayList<String>();
		...
		try {
			...
			if (ok && (selected != null)) {
				...
				for (IBond bond : mol.bonds())
					if (!"H".equals(bond.getAtom(0).getSymbol())
	...
	public boolean verifyRule(IAtomContainer mol)
		...
		if ((mol.getAtomCount() == (c + h + o)) && (o > 0)) {
			...
			if ((rings != null) && (rings.getAtomContainerCount() > 0)) {
				if (mf.isAromatic()) {
					...
					for (int i = 0; i < rings.getAtomContainerCount(); i++) {
						...
						if ((ring.getAtomCount() < 5)
								|| (ring.getAtomCount() > 6)) {
							logger.fine("Ring but not 5 or 6 membered\t"
									+ MSG_NOTACOMMONCARBOHYDRATE);
							return false;
						}
					}
				// cyclic, should have only alcohol or ether groups
				list = FunctionalGroups.getUniqueBondMap(mol,
						getSubstructure(index_alcohol), true);
				if ((list == null) || (list.size() == 0)) {
					logger.fine(MSG_HASGROUP
							+ getSubstructure(index_alcohol).getID() + MSG_NO
							+ MSG_NOTACOMMONCARBOHYDRATE);
					return false;
				} else if ((list.size() < 3)) {
					logger.fine(MSG_HASGROUP
					...
					FunctionalGroups.markMaps(mol,
							getSubstructure(index_alcohol), list);
				// ether
				list = FunctionalGroups.getUniqueBondMap(mol,
						getSubstructure(index_ether), true);
				if ((list == null) || (list.size() == 0)) {
					logger.fine(MSG_HASGROUP
							+ getSubstructure(index_ether).getID() + MSG_NO
							+ MSG_NOTACOMMONCARBOHYDRATE);
					return false;
				} else {
					FunctionalGroups.markMaps(mol,
							getSubstructure(index_ether), list);
				}
				// and nothing else
			...
			} else { // acyclic should have only aldehyde, ketone and alcohol
						// groups
				...
				// one aldehyde group
				list = FunctionalGroups.getUniqueBondMap(mol,
						getSubstructure(index_aldehyde), true);
				if ((list == null) || (list.size() == 0)) {
					logger.fine(MSG_HASGROUP
							+ getSubstructure(index_aldehyde).getID() + MSG_NO);
				} else if (list.size() > 1) {
					logger.fine(MSG_MANYGROUPSOFAKIND
							+ getSubstructure(index_aldehyde).getID()
							+ MSG_NOTACOMMONCARBOHYDRATE);
					return false;
				} else
					FunctionalGroups.markMaps(mol,
							getSubstructure(index_aldehyde), list);
				// or one ketone group
				list = FunctionalGroups.getUniqueBondMap(mol,
						getSubstructure(index_ketone), true);
				if ((list == null) || (list.size() == 0)) {
					logger.fine(MSG_HASGROUP + getSubstructure(index_ketone)
							+ MSG_NO);
				} else if (list.size() > 1) {
					logger.fine(MSG_MANYGROUPSOFAKIND
							+ getSubstructure(index_ketone).getID()
							+ MSG_NOTACOMMONCARBOHYDRATE);
					return false;
				} else
					FunctionalGroups.markMaps(mol,
							getSubstructure(index_ketone), list);
