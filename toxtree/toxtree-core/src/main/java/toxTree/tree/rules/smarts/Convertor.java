...
/**
 * Abstract class that provides convertor procedures to convert CDK classes to
 * JOELib classes and visa versa. Fixed from CDK cConvertor class.
 * 
 * <p>
 * JOELib is a Java implementation of the OELib classes and can be found at: <a
 * href="http://joelib.sourceforge.net/">http://joelib.sourceforge.net/</a>.
 * 
...
public class Convertor {
	 ...
	 * class.
	 * 
	 * Conversion includes: - atomic number - coordinates
	 * 
	 * @param atom
	 ...
	 * 
	 * @param atom
	 *            CDK atom to be converted
	 * @param coordType
	 *            coordinates to use. if -1 this converter uses the available
	...
	public static JOEAtom convert(IAtom atom, int coordType) {
		if (atom != null) {
			...
			if (coordType == COORDINATES_3D
				...
				convertedAtom.setVector(atom.getPoint3d().x,
						atom.getPoint3d().y, atom.getPoint3d().z);
			} else if (coordType == COORDINATES_2D
					|| (atom.getPoint2d() != null && coordType != -1)) {
				convertedAtom.setVector(atom.getPoint2d().x,
						atom.getPoint2d().y, 0.0);
			} else {
			...
			if (atom.getAtomicNumber() <= 0)
				try {
					org.openscience.cdk.config.IsotopeFactory isotopeFactory = Isotopes
							.getInstance();
	...
	public static IAtom convert(JOEAtom atom) {
		if (atom != null) {
			IAtom convertedAtom = MoleculeTools.newAtom(
			...
			try {
				...
				IElement element = isotopeFactory.getElement(atom
						.getAtomicNum());
				convertedAtom = MoleculeTools.newAtom(
			...
			try {
				...
				convertedAtom.setPoint3d(new Point3d(atom.getVector().x(), atom
						.getVector().y(), atom.getVector().z()));
			} catch (java.lang.Exception exception) {
	 ...
	 * class.
	 * 
	 * Conversion includes: - atoms which it conects - bond order
	 * 
	 * @param bond
	 *            class to be converted
	 * @return converted class in JOELib
	...
	public static IBond convert(JOEBond bond) {
		if (bond != null) {
			...
			IBond bond_cdk = new Bond(convert(bond.getBeginAtom()),
					convert(bond.getEndAtom()), order);
	 ...
	 * 
	 * Conversion includes: - atoms - bonds
	 * 
	 * @param mol
			...
			for (int i = 0; i < NOatoms - 1; i++) {
				for (int j = i + 1; j < NOatoms; j++) {
					if (matrix[i][j] != 0.0) {
						...
						/*
						 * if (mol.getBond(mol.getAtom(i),
						 * mol.getAtom(j)).getFlag(CDKConstants.ISAROMATIC))
	...
	public static IAtomContainer convert(JOEMol mol) {
		if (mol != null) {
			IAtomContainer converted = SilentChemObjectBuilder.getInstance()
