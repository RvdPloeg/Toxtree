...
public class Introspection {
	...
	public static File[] enumerateJars(File directory)
			throws toxTree.exceptions.IntrospectionException {
		
		InputStream in = null;
		try {
			logger.info("Looking for .jar files in\t"
			...
			for (int i = 0; i < keys.length; i++) {
				if ("folder".equals(keys[i].toString())) continue;
				File file = new File(folder + "/" +plugins.getProperty(keys[i].toString()));
				if (file.exists()) {
	...
	public static void configureURLLoader(ClassLoader classLoader) {
		...
		for (int i = 0; i < defaultLocation.length; i++) {
			try {
				configureURLLoader(new File(getToxTreeRoot()
						+ defaultLocation[i]));
	 ...
	 * @throws toxTree.exceptions.IntrospectionException
	 */
	public static void configureURLLoader(File directory)
			throws toxTree.exceptions.IntrospectionException {
		if (loader == null)
	 ...
	 * @throws toxTree.exceptions.IntrospectionException
	 */
	public static void configureURLLoader(File[] jars)
			throws toxTree.exceptions.IntrospectionException {
		if (loader == null)
			throw new IntrospectionException("Class loader not set!");
		for (int i = 0; i < jars.length; i++)
	...
	public static ToxTreePackageEntries implementInterface(
			ClassLoader classLoader, File directory, String interfaceName)
			throws toxTree.exceptions.IntrospectionException {
		setLoader(loader);
		...
		for (int i = 0; i < files.length; i++)
			...
			try {
				logger.info("Inspecting jar file\t" + files[i] + "\t"
						+ files[i].toURL());
				...
				while (entries.hasMoreElements()) {
					...
					if (rule != null) {
						logger.fine("Class\t" + name + "\timplements\t"
								+ interfaceName + "\tYES");
						try {

							if (!name.endsWith("BatchDecisionResultsList"))
								rules.add(new ToxTreePackageEntry(name,
	 ...
	 *         {@link IDecisionRule}
	 */
	public static ToxTreePackageEntries getAvailableRuleTypes(
	 ...
	 */
	public static ToxTreePackageEntries getAvailableTreeTypes(
			ClassLoader classLoader) {
	 ...
	 *         {@link ArrayList}
	 */
	public static ToxTreePackageEntries getAvailableTypes(
			ClassLoader classLoader, String interfacename) {
		setLoader(classLoader);
		ToxTreePackageEntries listAll = null;

		for (int i = 0; i < defaultLocation.length; i++) {
			try {
				ToxTreePackageEntries list = implementInterface(classLoader,
						new File(getToxTreeRoot() + defaultLocation[i]),
						interfacename);
				if (list.size() != 0) {
					if (listAll == null)
	 ...
	 */
	public static ToxTreePackageEntries getAvailableCategoryTypes(
			ClassLoader classLoader) {
		...
		if ((loader == null) || !(loader instanceof ToxTreeClassLoader)) {
			...
			for (int i = 0; i < defaultURL.length; i++) {
				try {
					...
					defaultURL[i] = new URL("jar", "", getToxTreeRoot()
							+ defaultLocation[i] + "!/");
	 ...
	 */
	public static IDecisionMethod loadRules(InputStream stream, String newTitle)
		...
		try {
			...
			throw new IntrospectionException(
					"Error when loading the decision tree " + newTitle, e);
	...
	public static IDecisionMethodsList loadForest(InputStream stream)
			throws IntrospectionException {
		ToxTreeObjectInputStream in = null;
		try {
			...
			IDecisionMethodsList methods = (IDecisionMethodsList) in
					.readObject();
			/*
			...
			throw new IntrospectionException(
					"Error when loading the decision forest", e);
		} finally {
			try {
				if (in != null)
					in.close();
	...
	public static void saveRulesXML(IDecisionMethod method, OutputStream out)
			throws IntrospectionException {
		Thread.currentThread().setContextClassLoader(Introspection.getLoader());
		XMLEncoder encoder = new XMLEncoder(out);
		encoder.setPersistenceDelegate(ProgressStatus.STATUS.class,
	...
	public static IDecisionMethod loadRulesXML(InputStream stream,
			String newTitle) throws IntrospectionException {
		try {
			...
			XMLDecoder decoder = new XMLDecoder(
					new BufferedInputStream(stream), null, el, loader);
	...
	public static IDecisionMethodsList loadForestXML(InputStream stream)
			throws IntrospectionException {
		try {
			if (loader == null)
				setLoader(null); // this will trigger creation of the right
			...
			XMLDecoder decoder = new XMLDecoder(
					new BufferedInputStream(stream), null, null, loader);
			IDecisionMethodsList methods = (IDecisionMethodsList) decoder
					.readObject();
			decoder.close();
			/*
			 * JOptionPane.showMessageDialog(null, methods.size() +
			...
			throw new IntrospectionException(
					"Error when loading the decision forest from .fml file", e);
		}
	}
	...
	protected Expression instantiate(Object oldInstance, Encoder out) {
		...
		if (!Modifier.isPublic(type.getModifiers()))
			throw new IllegalArgumentException(
					"Could not instantiate instance of non-public class: "
							+ oldInstance);
		...
		for (Field field : type.getFields()) {
			...
			if (Modifier.isPublic(mod) && Modifier.isStatic(mod)
				...
				try {
					...
					throw new IllegalArgumentException(
							"Could not get value of the field: " + field,
							exception);
				}
			}
		}
		throw new IllegalArgumentException("Could not instantiate value: "
				+ oldInstance);
	}
