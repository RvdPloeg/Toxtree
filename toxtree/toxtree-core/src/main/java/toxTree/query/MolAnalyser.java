...
public class MolAnalyser {
	protected static Logger logger = Logger.getLogger(MolAnalyser.class
			.getName());
	 ...
	 * System.out.println("mol builder "+
	 * mol.getBuilder().getClass().getName()); for (IBond bond: mol.bonds())
	 * System
	 * .out.println(String.format("%s bond builder %s",bond.getOrder(),bond
	 * .getBuilder().getClass().getName()));
	 * 
	 * for (IAtom atom: mol.atoms())
	 * System.out.println(String.format("%s %s implicit H %d atom builder %s"
	 * ,atom
	 * .getSymbol(),atom.getAtomTypeName(),atom.getImplicitHydrogenCount(),atom
	 * .getBuilder().getClass().getName()));
	 * 
	 * 
	 * }
	...
	public static void analyse(IAtomContainer mol) throws MolAnalyseException {
		...
		if (mol == null)
			throw new MolAnalyseException("Null molecule!", mol);
		if (mol.getAtomCount() == 0)
			throw new MolAnalyseException("No atoms!", mol);

		try {
			// New - 16 Jan 2008 - configure atom types
			logger.fine("Configuring atom types ...");
			CDKAtomTypeMatcher matcher = CDKAtomTypeMatcher.getInstance(mol
					.getBuilder());
			Iterator<IAtom> atoms = mol.atoms().iterator();
			while (atoms.hasNext()) {
				IAtom atom = atoms.next();
				IAtomType type = matcher.findMatchingAtomType(mol, atom);
				// CDK 1.5 no longer uses NULL for
				// "no idea what atom type this is", but uses the "X" atom type
				// so, the next line restores the CDK 1.4 "functionality"
				if ("X".equals(type.getAtomTypeName()))
					type = null;
				try {
					...
					if ("*".equals(atom.getSymbol())
							|| "R".equals(atom.getSymbol()))
			...
			while (atoms.hasNext()) {
				...
				if ((atom.getAtomicNumber() == null)
					...
					Integer no = PeriodicTable
							.getAtomicNumber(atom.getSymbol());
					if (no != null)
			...
			MolFlags mf = null;
			if ((o != null) && (o instanceof MolFlags))
				mf = (MolFlags) o;
			else
				mf = null;
			if (mf == null)
			...
			try {
				ringSet = Cycles.all(mol).toRingSet();
			} catch (Intractable x) {
				// will try SSSR
				logger.warning(x.getMessage());
				ringSet = Cycles.sssr(mol).toRingSet();
			}
			int size = ringSet.getAtomContainerCount();
			...
			if (size == 0) {
				...
				for (int i = 0; i < ringSet.getAtomContainerCount(); i++) {
					...
					// boolean isaromatic =
					// ring.getFlag(CDKConstants.ISAROMATIC);
					...
					for (int j = 0; j < ring.getAtomCount(); j++) {
						...
						if (a.getFlag(CDKConstants.ISAROMATIC))
							aromatic_atoms++;
						/*
						 * This flag should already be set if (isaromatic)
					...
					boolean isaromaticRing = (aromatic_atoms == ring
							.getAtomCount());
					if (heterocyclicRing) {
			...
			/*
			 * HydrogenAdder h = new HydrogenAdder(); try { if (mol instanceof
			 * Molecule)
			 * h.addExplicitHydrogensToSatisfyValency((IAtomContainer)mol); else
			 * { IAtomContainerSet moleculeSet =
			 * ConnectivityChecker.partitionIntoMolecules(mol);
			 * 
			 * for (int k = 0; k < moleculeSet.getMoleculeCount(); k++) {
			 * IAtomContainer molPart = moleculeSet.getMolecule(k); for (int i =
			 * 0; i < molPart.getAtomCount(); i++) {
			 * h.addExplicitHydrogensToSatisfyValency(molPart,
			 * molPart.getAtom(i), mol); } } }
			 * 
			 * } catch (IOException x) { throw new MolAnalyseException(x); }
			 * catch (ClassNotFoundException x) { throw new
			 * MolAnalyseException(x); } catch (CDKException x) { throw new
			 * MolAnalyseException(x); }
			 */
