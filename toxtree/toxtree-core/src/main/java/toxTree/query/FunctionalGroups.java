...
Contact: jeliazkova.nina2@gmail.com
...
public class FunctionalGroups {
	...
	public static QueryAtomContainer anhydride() {
		...
		SymbolQueryAtom o = new SymbolQueryAtom(MoleculeTools.newAtom(
				query.getBuilder(), Elements.OXYGEN));

		SymbolQueryAtom r = new SymbolQueryAtom(MoleculeTools.newAtom(
				query.getBuilder(), Elements.CARBON));
		r.setProperty(DONTMARK, query.getID());
			...
			for (int i = 0; i < rings.getAtomContainerCount(); i++) {
				...
				if (esterGroups.size() == 2) {
					...
					for (int a = 0; a < ring.getAtomCount(); a++)
						if ("C".equals(ring.getAtom(a).getSymbol()))
							continue;
						else if ("O".equals(ring.getAtom(a).getSymbol())) {
		...
		for (int j = 0; j < list.size(); j++) {
			...
			for (int i = 0; i < thisMap.size(); i++) {
				...
				if (mappedBonds[rmap.getId1()] > 0) {
					...
					logger.info(b2.getAtom(0) + "\t"
							+ b2.getAtom(0).getSymbol());
					if ("R".equals(b2.getAtom(0).getSymbol()))
						continue;
					if ("R".equals(b2.getAtom(1).getSymbol()))
						continue;
	...
	public static int markCHn(IAtomContainer mol) {
		...
		for (int i = 0; i < mol.getAtomCount(); i++) {
			...
			for (int j = 0; j < neighbors.size(); j++) {
				...
				if ("H".equals(n.getSymbol()))
					hydrogens++;
				else if ("C".equals(n.getSymbol()))
					carbons++;
			}
	...
	public static StringBuilder mapToString(IAtomContainer mol) {
		...
		for (int i = 0; i < mol.getAtomCount(); i++) {
			...
			for (Iterator e = a.getProperties().keySet().iterator(); e
					.hasNext();) {
				Object key = e.next();
	...
	public static IAtomContainer createAtomContainer(String smiles,
			boolean addHydrogens, String id) throws CDKException,
