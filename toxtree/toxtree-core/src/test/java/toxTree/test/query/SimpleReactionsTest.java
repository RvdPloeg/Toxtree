...
public class SimpleReactionsTest {
	...
	protected IAtomContainerSet processHydrolysis(IAtomContainer mol, int index) throws Exception {
		...
		if (sc != null) {
			...
			/*
			 * Reaction r = new Reaction(); r.addReactant((IAtomContainer)mol);
			 * for (int p=0; p< sc.getAtomContainerCount(); p++)
			 * r.addProduct((IAtomContainer)sc.getAtomContainer(p));
			 * StringWriter writer = new StringWriter(); CMLWriter cmlWriter =
			 * new CMLWriter(writer); try { cmlWriter.write(r); } catch
	...
	public void testHydrolysis4() throws Exception {
		...
		Assert.assertEquals(2, results.getAtomContainerCount());
		/*
		 * mol = FunctionalGroups.createAtomContainer("O=P(O)(OCC)OCC"); results
		 * = processHydrolysis(mol,index); assertNotNull(results);
		 */
	...
	protected void printProducts(IAtomContainerSet products, IReaction reaction) throws Exception {
		...
		/*
		 * 
		 * SmilesGenerator g = new SmilesGenerator(); logger.debug("");
		 * logger.debug(reaction.getID());
		 * logger.debug(g.createSMILES((Reaction) reaction));
		 * logger.debug("Products"); if (products == null) return; for (int p=0;
		 * p< products.getAtomContainerCount(); p++)
		 * System.out.println((p+1)+".\n"+
		 * FunctionalGroups.mapToString((IAtomContainer
		 * )products.getAtomContainer(p))); logger.debug((p+1)+".\t"+
		 * g.createSMILES((IAtomContainer)products.getAtomContainer(p)));
		 */
	...
	public void testMultipleReactions() throws Exception {
		...
		IAtomContainer Gr3 = (IAtomContainer) FunctionalGroups
				.createAtomContainer(
