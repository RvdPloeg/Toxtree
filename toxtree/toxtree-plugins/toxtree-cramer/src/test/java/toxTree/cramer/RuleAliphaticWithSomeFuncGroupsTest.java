...
public class RuleAliphaticWithSomeFuncGroupsTest extends AbstractRuleTest {
	...
	public void test() throws Exception {
		Object[][] answer = { { "CC(=O)CCC(C)=O", new Boolean(false) }, // 20N
				...
				{ "CCOCCOCCOCCCCCCCCCCCC", new Boolean(true) }, // SLTS
																// unsulphated"
		};
	...
	public void testMunroExample() throws Exception {
		Object[][] answer = { { "CN(C)C(=S)SCSC(=S)N(C)C", new Boolean(true) } // fails
																				// (Munro
																				// )
		};
	...
	public void testBugOverlapingGroups() throws Exception {
		...
		Object[][] answer = { { "COCCOC(=O)C=C", new Boolean(true) },
				{ "C=CC(=O)OCCOC", new Boolean(true) }

		};
		// 1N,2N,3N,5N,6N,7N,16N,17N,19Y,20Y,21N,18N
		IAtomContainer mol1 = (IAtomContainer) FunctionalGroups
				.createAtomContainer((String) answer[0][0]);
		IAtomContainer mol2 = (IAtomContainer) FunctionalGroups
				.createAtomContainer((String) answer[1][0]);
	...
	/*
	 * 
	 * public void testBugAnyAtom() throws Exception { //two smiles of the same
	 * compound Object[][] answer = { {"CCOC=O",new Boolean(true)},
	 * 
	 * }; Preferences.setProperty(Preferences.SMILESPARSER,"true");
	 * IAtomContainer mol1 = (IAtomContainer)
	 * FunctionalGroups.createAtomContainer((String)answer[0][0]);
	 * System.out.println("OpenBabel"); MolAnalyser.analyse(mol1); int i=0; for
	 * (IAtom atom : mol1.atoms()) {
	 * System.out.println(String.format("%s\t%s",atom
	 * .getSymbol(),atom.getMassNumber())); i++;
	 ...
	 * 
	 * QueryAtomContainer ester = FunctionalGroups.ester(); i=0; for (IAtom atom
	 * : ester.atoms()) { atom.setID(Integer.toString(i)); i++; } for (IBond
	 * bond: ester.bonds())
	 * System.out.println(String.format("Ester\t%s\t%s\t(%s.%s.%s,%s.%s.%s)"
	 * ,bond.getOrder(),bond.getFlag(CDKConstants.ISAROMATIC),
	 * bond.getAtom(0).getID
	 * (),bond.getAtom(0).getClass().getName(),bond.getAtom(0).getSymbol(),
	 * bond.
	 * getAtom(1).getID(),bond.getAtom(1).getClass().getName(),bond.getAtom(
	 * 1).getSymbol()));
	 * 
	 ...
	 * 
	 * 
	 * Preferences.setProperty(Preferences.SMILESPARSER,"false"); IAtomContainer
	 * mol2 = (IAtomContainer)
	 * FunctionalGroups.createAtomContainer((String)answer[0][0]);
	 * System.out.println("CDK"); MolAnalyser.analyse(mol2); i = 0; for (IAtom
	 * atom : mol2.atoms()) {
	 * System.out.println(String.format("%s\t%s",atom.getSymbol
	 * (),atom.getMassNumber())); i++; atom.setID(Integer.toString(i)); } for
	 * (IBond bond: mol2.bonds())
	 * System.out.println(String.format("%s\t%s\t(%s.%s,%s.%s)"
	 * ,bond.getOrder(),bond.getFlag(CDKConstants.ISAROMATIC),
	 * bond.getAtom(0).getID(),bond.getAtom(0).getSymbol(),
	 * bond.getAtom(1).getID(),bond.getAtom(1).getSymbol()));
