 ...
 */
public class RuleRingComplexSubstituents30 extends
	 ...
	 * 
	 */
	public RuleRingComplexSubstituents30() throws Exception {
		super();
		...
		explanation
				.append("Disregarding ring hydroxy or methoxy does the ring bear substituents <i>other</i> than 1-5-carbon <i>aliphatic</i> (A) groups,");
		explanation
				.append("either hydrocarbon or containing alcohol, ketone, aldehyde, carboxyl or simple esters that may be hydrolised to ring substituents of <=5 carbons? ");
		explanation
				.append("(If a simple ester that may be hydrolised, treat the aromatic portion by Q.18 and the residue by Q19.<p>");
		explanation
				.append("This should be answered NO if the ring bears only aliphatic groups of <=5 carbons, which are either hydrocarbon in nature or contain the groups listed.");
		explanation
				.append("If the ring bears any other substituents than those listed, the question should be answered YES and proceed to Q31.");
		explanation.append("</html>");
	...
	public boolean substituentIsAllowed(IAtomContainer a, int[] place)
		...
		if (rs.getAtomContainerCount() > 0) {
			...
			int c = FunctionalGroups.getLongestCarbonChainLength(a); // to
																		// verify
																		// if
																		// the
																		// chain
			...
			if (c > 5) {
				 ...
				 * SimpleReactions sr = new SimpleReactions(); try { //trying to
				 * hydrolize b = true; SetOfAtomContainers residues =
				 * sr.isReadilyHydrolised((IAtomContainer)a); canBeHydrolized =
				 * (residues != null); if (residues == null) {
				 * logger.debug("Long chain substituent can't be hydrolized!\t - "
				 * ,c); b = false; } else for (int i = 0; i <
				 * residues.getAtomContainerCount(); i++) {
				 * mf.analyseAtomContainer(residues.getAtomContainer(i)); c =
				 * mf.getAtomCount(cAtom); if (c > 5) { b = false;
				 * logger.debug("Residue with > 5 carbon atoms!\t - ",c); break;
				 * } } } catch (ReactionException x) { throw new
				 * DecisionMethodException(x); }
				 * 
				 * } else { logger.debug(
				 * "Ring substituent:\tNo ester group found, will not try hydrolysis."
				 * ); b = false; }
				 */
				b = false;
			} else
	 ...
	 * 
	 * @see
	 * toxTree.tree.rules.RuleRingOtherThanAllowedSubstituents#verifyRule(org
	 * .openscience.cdk.Molecule)
	 */
	@Override
		...
		if (FunctionalGroups.hasGroup(mol, ester, false)) {
			...
			try {
				...
				if (!canBeHydrolized) {
					...
					for (int i = 0; i < residues.getAtomContainerCount(); i++)
						try {
							IAtomContainer residue = residues
									.getAtomContainer(i);
							MolAnalyser.analyse(residue);
							MolFlags mf = (MolFlags) residue
							...
							if (mf.isAromatic()) {
								...
								// since here we have a single ring, this is the
							...
							throw new DecisionMethodException(
									"Error when preprocessing residues!", x);
						}
