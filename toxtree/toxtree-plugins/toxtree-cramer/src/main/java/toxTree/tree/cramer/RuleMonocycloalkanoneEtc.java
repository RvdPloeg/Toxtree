...
public class RuleMonocycloalkanoneEtc extends RuleOnlyAllowedSubstructures {
	 ...
	 * 
	 */
	public RuleMonocycloalkanoneEtc() throws Exception {
		super();
		...
		explanation
				.append("Does the structure contain no functional groups other than those listed in Q24 and is either a monocycloalkanone or a bicyclic compound with or without a ring ketone?");
		explanation.append("</html>");
	...
	@Override
	public boolean verifyRule(IAtomContainer mol, IAtomContainer selector)
			throws DecisionMethodException {
		...
		if (rings.getAtomContainerCount() == 1) {
			// monocycloalkanone
			if (selector != null)
				selectRings(rings, selector);
			if (mf.isHeterocyclic()) {
				logger.fine("Heterocyclic ring found");
				return false;
			}
			return ((FunctionalGroups.hasGroup(mol,
					FunctionalGroups.ketone_ring())));
		}
