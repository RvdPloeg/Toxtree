...
public class RuleSomeBenzeneDerivatives extends RuleRingAllowedSubstituents {
	 ...
	 * 
	 */
	public RuleSomeBenzeneDerivatives() throws Exception {
		super();
		setAnalyzeAromatic(true);
		setAnalyzeOnlyRingsWithFlagSet(true);
		chIDs = new ArrayList();
		chIDs.add(FunctionalGroups.CH);
		chIDs.add(FunctionalGroups.CH2);
		chIDs.add(FunctionalGroups.CH3);
		...
		alkoxyIDs = new ArrayList();
		alkoxyIDs.add(FunctionalGroups.ETHER);
		alkoxyIDs.add(FunctionalGroups.CH);
		alkoxyIDs.add(FunctionalGroups.CH2);
		alkoxyIDs.add(FunctionalGroups.CH3);
		...
		explanation
				.append("<HTML>Is the substance a benzene derivative bearing substituents consisting only of <UL>");
		explanation
				.append("<LI>(a)hydrocarbon chains or 1'hydroxy or hydroxy ester-substituted hydrocarbon chains and");
		explanation
				.append("<LI>(b)one or more alkoxy groups, one of which must be para to the hydrocarbon chain in (a)?");
		explanation
				.append("</UL>this places in class III safrole, myristicin and related substances.</html>");
	...
	protected IRingSet hasRingsToProcess(IAtomContainer mol)
		...
		if (benzene == 1) {
			...
			if (benzene > 1)
				logger.fine("Too many benzene rings found.");
			else
				logger.fine("No benzene rings found.");
			return null;
		}
	}
	...
	public boolean substituentIsAllowed(IAtomContainer a, int[] place)
		...
		if (FunctionalGroups.hasMarkedOnlyTheseGroups(a, chIDs)) {
			for (int j = 0; j < place.length; j++)
				if (place[j] >= 0)
					places[place[j]] = index_hydrocarbon;
			logger.fine(MSG_BENZENESUBSTITUENT
					+ "hydrocarbon chain\tat place\t" + place);
			return true;
		}
		...
		if (list != null) {
			...
			if (FunctionalGroups.hasMarkedOnlyTheseGroups(a, hydroxyIDs)) {
				for (int j = 0; j < place.length; j++)
					if (place[j] >= 0)
						places[place[j]] = index_hydroxy1;
				logger.fine(MSG_BENZENESUBSTITUENT
						+ "1'-hydroxy hydrocarbon chain");
				return true;
			}
		}// clear hydroxy mark ?
		/*
		 * q =
		 * (QueryAtomContainer)getSubstructure(index_hydroxyEsterSubstituted);
		 * list = FunctionalGroups.getBondMap(a,q,true); if (list != null) {
		 * //FunctionalGroups.markMaps(a,q,list); if
		 * (FunctionalGroups.hasMarkedOnlyTheseGroups(a,hydroxyIDs)) { for (int
		 * j=0;j<place.length;j++) if (place[j] >=0) places[place[j]] =
		 * index_hydroxyEsterSubstituted; logger.fine(MSG_BENZENESUBSTITUENT,
		 * "hydroxy ester substituted hydrocarbon chain"); return true; } }
		 */
		...
		if (FunctionalGroups.hasMarkedOnlyTheseGroups(a, hydroxyIDs)) {
			for (int j = 0; j < place.length; j++)
				if (place[j] >= 0)
					places[place[j]] = index_ester;
			logger.fine(MSG_BENZENESUBSTITUENT
					+ "hydroxy ester substituted hydrocarbon chain");
			return true;
		}
		...
		if (list != null) {
			...
			if (FunctionalGroups.hasMarkedOnlyTheseGroups(a, alkoxyIDs)) {
				for (int j = 0; j < place.length; j++)
					if (place[j] >= 0)
						places[place[j]] = index_alkoxy;

				logger.fine(MSG_BENZENESUBSTITUENT + "alkoxy group at place\t"
						+ place);
				return true;
			}
		}
