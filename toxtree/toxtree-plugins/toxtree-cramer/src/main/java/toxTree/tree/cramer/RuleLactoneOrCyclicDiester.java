...
/**
 * Rule 8 of the Cramer scheme (see {@link toxTree.tree.cramer.CramerRules})
 * 
...
public class RuleLactoneOrCyclicDiester extends RuleSubstructures {
	 ...
	 * 
	 */
	public RuleLactoneOrCyclicDiester() throws Exception {
		super();
		...
		explanation
				.append("<html>Is it a lactone or cyclic diester?<p>This question separates the lactones and cyclic diesters from other heterocyclic compounds.</html>");
	...
	public boolean verifyRule(IAtomContainer mol)
		...
		// if (FunctionalGroups.isCyclicDiester(mol,mf.getRingset())) return
		// true;
		List list = FunctionalGroups.getBondMap(mol,
				getSubstructure(index_lactone), false);
		if ((list != null) && (list.size() > 0)) {// lactone found
			...
			FunctionalGroups
					.markMaps(mol, getSubstructure(index_lactone), list);
			...
			for (int i = 0; i < rings.getAtomContainerCount(); i++) {
				...
				if ((ring.getAtomCount() > 3) /* && (ring.getAtomCount() < 7) */) {
					...
					for (int j = 0; j < ring.getBondCount(); j++) {
						if (ring.getBond(j).getProperty(
								FunctionalGroups.LACTONE) != null) {
