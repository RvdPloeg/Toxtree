...
public class DescriptorMRTest {
	...
	public void test() throws Exception {
		...
		Object[][] test = new Object[][] { { "C=CC=O", new Double(1.655) },
		...
		for (int i = 0; i < test.length; i++) {
			IAtomContainer ac = FunctionalGroups.createAtomContainer(test[i][0]
					.toString());
			AtomContainerManipulator.percieveAtomTypesAndConfigureAtoms(ac);
			CDKHueckelAromaticityDetector.detectAromaticity(ac);

			DoubleResult r = (DoubleResult) ((DescriptorValue) mr.calculate(ac))
					.getValue();
			Assert.assertEquals(((Double) test[i][1]).doubleValue() * 10,
					r.doubleValue(), 1E-1);
