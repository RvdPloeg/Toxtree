...
/**
 * Returns true if structure is predicted as mutagen , based on linear
 * discriminant analysis. AM1
 * 
 * <pre>
 ...
 * </pre>
 * 
 * 21 Sep 2007 - the equation was changed (RE Prof. R.Benigni) in order to use
 * PM3 instead of AM1 calculations.
 * 
 * <pre>
...
public class RuleDAMutagenicityAromaticAmines extends LinearDiscriminantRule {
	...
	/*
	 * old explanation b.append(
	 * "The indicator variable Idist is a structural parameter coding for the presence (Idist = 1, otherwise Idist = 0) of substituents on the positions 3'-, 4'- and 5'- of 4-aminobiphenyl,"
	 * ); b.append(
	 * "<br>and position 7- of 2-aminofluorene (e.g.: 4'-nButyl-4-aminobiphenyl; 4'-tButyl-4-aminobiphenyl;	4'-Trifluoromethyl-4-aminobiphenyl; 3'-Trifluoromethyl-4-aminobiphenyl)."
	 * );
	 */
	b.append("</ul>");
	// [*]C1=CC(=CC([*])=C1([*]))C2=CC=C(N)C=C2 substituents on the
	// positions 3-, 4- and 5- of 4-aminobiphenyl
	// [*]C1=CC=C3C(=C1)C=C2CC(N)C=CC23 subst at position 7- of
	// 2-aminofluorene
	...
	String[][] nd = {
		{ "EHOMO", "PM3 molecular orbital energies for the Highest Occupied Molecular Orbital (HOMO)",
			"Verify if OpenMopac is available and properly configured" },
		{ "ELUMO", "PM3 molecular orbital energies for the Lowest Unoccupied Molecular orbital (LUMO)",
			"Verify if OpenMopac is available and properly configured" },
		{ "MR2", "MR2 is the MR contribution of substituents in position 2 to the amino group.",
			reason_notfound },
		{ "MR3", "MR3 is the MR contribution of substituents in position 3 to the amino group.",
			reason_notfound },
		{ "MR6", "MR6 is the MR contribution of substituents in position 6 to the amino group.",
			reason_notfound }, { "Idist", "Indicator for crowded position", "?" }, };
	...
	// double weights[] = new double[] {- 2.85,1.84,0.70, 0.69, 1.90,
	// 3.36,0};
	...
	LinearQSARModel model = new LinearQSARModel(names, descriptors, result, weights) {
	    /**
		     * 
		     */
	    private static final long serialVersionUID = 9207642012501062481L;
	    ...
	    @Override
	    protected void processUnavailableDescriptors(String name) throws QSARModelException {
		throw new QSARModelException(dictionary.get(name));
	    }
	};

	/**
	 * The following are the correction factors to be applied to the Toxtree
	 * variables, before they are entered into the final equation for the
	 * calculation of w (thus, an intermediate step is necessary between
	 * calculation of the variables and calculation of w. The values to be
	 * shown are those corrected).
	 * 
	 * 
	 * homo_s = ehomo_tt * 1.03383 + 0.30348; lumo_s = elumo_tt * 0.98963 -
	 * 0.04037;
	 * 
	 * The variables "_s" are the final (corrected) values of the variables;
	 * the variables "_tt" are the variables calculated by Toxtree.
	 */
	IDescriptorPreprocessor p = null;
	try {
	    p = new LinearPreprocessor(new double[] { 1.03383, 0.98963, 1, 1, 1, 1 }, new double[] { 0.30348, -0.04037,
		    0, 0, 0, 0 });
	    ((LinearPreprocessor) p)
		    .setExplanation("The following are the correction factors to be applied to the Toxtree variables, before they are entered into the final equation for the calculation of W.");
	} catch (Exception x) {
	 ...
	 * c.setExplanation(x.getMessage()); return new
	 * DRuleException(x,c,false);
	 */
	return new DRuleException(x, new CategoryError("Error when processing " + getID() + ": " + x.getMessage()),
		false);
