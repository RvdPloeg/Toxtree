 ...
 * presence (Idist = 1, otherwise Idist = 0) of crowded substituents on the
 * positions 3'-, 4'- and 5'- of 4-aminobiphenyl. <br>
 * In particular, Methyl, Ethyl or Propyl substituents are not defined as
 * crowded, whilst tri-substituted methyl and butyl (or bigger) substituents are
 * defined as crowded substituents. <br>
 * For the other functional groups, those who have Partial MR values >= 1.96
 * (butyl), should have Idist =1. <br>
 * Please note that trifluoromethyl has Idist=1 irrespective of the MR value
 * (0.5)
 * 
...
public class DescriptorIDist implements IMolecularDescriptor {
	...
	public DescriptorIDist() throws Exception {
		...
		smarts[0] = new SmartsPatternCDK(
				"c(C([A&!#1])([A&!#1])([A&!#1]))ccc!@c1ccc([NH2])cc1");
		smarts[1] = new SmartsPatternCDK(
				"cc(C([A&!#1])([A&!#1])([A&!#1]))cc!@c1ccc([NH2])cc1");
		smarts[2] = new SmartsPatternCDK(
				"c(C([A&!#1][A&!#1][A&!#1]))ccc!@c1ccc([NH2])cc1");
		smarts[3] = new SmartsPatternCDK(
				"cc(C([A&!#1][A&!#1][A&!#1]))cc!@c1ccc([NH2])cc1");
		smarts[4] = new SmartsPatternCDK(
				"NC=1C=CC3=C(C=1)CC=2C=C([!#1])C=CC=23");
		// smarts[5] = new SmartsPatternCDK("cc(C)cc!@c1ccc([NH2])cc1");

		smarts[5] = new SmartsPatternCDK(
				"c([$(C3C1CC2CC(C1)CC3(C2))])ccc-c1ccc([NH2])cc1");
		smarts[6] = new SmartsPatternCDK(
				"cc([$(C3C1CC2CC(C1)CC3(C2))])cc-c1ccc([NH2])cc1");
	...
	public DescriptorValue calculate(IAtomContainer arg0) {
		if (smarts == null)
			return new DescriptorValue(getSpecification(), getParameterNames(),
					getParameters(), new IntegerResult(0),
	...
	public DescriptorSpecification getSpecification() {
		...
		return new DescriptorSpecification(b.toString(), this.getClass()
				.getName(),
