...
public class SubstituentExtractor {
	...
	public Hashtable<String, IAtomContainerSet> extractSubstituents(
		...
		for (int l = 0; l < list.size(); l++) {
			...
			if (markAtomsInRing(mark, (List) list.get(l), a, ringQuery)) {
				/*
				 * IAtom anchor = null; for (int i=0; i < a.getAtomCount();i++)
				 * { IAtom atom = a.getAtom(i);
				 * //System.out.println(atom.getSymbol() + "\t" +
				 * atom.getProperty(FunctionalGroups.RING_NUMBERING));
				 * SubstituentPosition position =
				 * SubstituentExtractor.getSubstituentNumber(mark,a,i); if
				 * (atom_C.equals(atom.getSymbol()) && (position != null) &&
				 * (position.getPosition() == 1)) anchor = atom; }
				 */
		...
		if (list != null) {
			for (int i = 0; i < list.size(); i++) {
				...
				if (map.getId2() >= 0) {
					...
					// mol.getAtom(map.getId1()).getSymbol() + " FUSED_RINGS=" +
					// mol.getAtom(map.getId1()).getProperty("FUSED_RINGS") +
					// " MR=" + mol.getAtom(map.getId1()).getProperty("MR") +
					// " RING=" +
					// q.getAtom(map.getId2()).getProperty(SubstituentExtractor._RING)
					// + " QID "+ q.getAtom(map.getId2()).getID() + " [ " +
					// map.getId1() + " <-> " + map.getId2() + " ] ");
			...
			for (int j = 0; j < b.getAtomCount(); j++) {
				...
				if (pos != null) {
					...
					if (!pos.isRing() && (pos.getPosition() != position)) {
						...
						// substituent
						// System.out.println("Found cycle "+position +
						// " "+pos);
						return true;
					}
				}
	...
	public static SubstituentPosition getSubstituentNumber(String property,
		...
		if (o == null)
			...
			logger.finer("Expected Integer type instead of "
					+ o.getClass().getName());
			return null;
		}
	}
