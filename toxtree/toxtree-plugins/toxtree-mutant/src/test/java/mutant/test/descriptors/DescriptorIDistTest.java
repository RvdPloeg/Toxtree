...
public class DescriptorIDistTest extends DescriptorsTest {
	 ...
	 * 
	 * Integer found = new Integer(1); Integer notfound = new Integer(0);
	 ...
	 * 
	 * {"c1cc(ccc1c2ccc(cc2)",notfound}, {"c1cc(ccc1OCc2ccc(cc2)",notfound},
	 * {"Nc1ccc(cc1)c2ccc(cc2)CCC",notfound} }; for (int i=0; i <
	 * test.length;i++) { IAtomContainer ac =
	 ...
	 * System.out.println(test[i][0]+"\tIdist="+r.intValue()); assertEquals(
	 * ((Integer)test[i][1]).intValue(), r.intValue());
	 * 
	 * 
	 * } }
	 */
	/*
	 * public void test() throws Exception { InputStream in = new
	 * FileInputStream("/data/aromatic_amines/ammTA100.csv");
	 * IteratingDelimitedFileReader reader = new
	 * IteratingDelimitedFileReader(in, new DelimitedFileFormat(',','"'));
	 * boolean ok = true; while (reader.hasNext()) { Object o = reader.next();
	 * if (o instanceof IAtomContainer) {
	 * MolAnalyser.analyse((IAtomContainer)o); String smiles =
	 * ((IAtomContainer)o).getProperty("SMILES").toString(); int idist =
	 * Integer.parseInt(((IAtomContainer)o).getProperty("Idist").toString());
	 * 
	 * DescriptorValue v = (DescriptorValue)
	 * descriptor.calculate((IAtomContainer)o);
	 * 
	 * if (idist != ((IntegerResult) v.getValue()).intValue()) {
	 * System.out.print
	 * (((IAtomContainer)o).getProperty("StructureID").toString());
	 * System.out.print("\t"); System.out.print(smiles);
	 * System.out.print("\tExpected\t"); System.out.print(idist);
	 * System.out.print("\tCalculated\t"); System.out.print(((IntegerResult)
	 * v.getValue()).toString()); System.out.println("\tBAD"); ok = false; } } }
	 * reader.close(); assertTrue(ok); }
	 */
	@Override
