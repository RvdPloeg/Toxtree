...
public class SA24 extends StructureAlertCDK {
	...
	public SA24() {
		...
		try {
			...
			addSubstructure(SA24_title,
					"[!$([#6](=O)[!O]),#1][C!H0;!R]([!$([#6](=O)[!O]),#1])!@;=[C!H0;!R]O[#6]");

			setID("SA24");
			setTitle(SA24_title);
			setExplanation(SA24_title
					+ "<br>An aromatic substituent on the oxygen is also allowed.<br>If a partial overlap with SA10 is found, this alert should not fire.");

			examples[0] = "COC=CC(C)=O"; // partial overlap with SA10
			// "C1CCC(OC)=CC1";
