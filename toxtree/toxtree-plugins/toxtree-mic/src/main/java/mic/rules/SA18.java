...
public class SA18 extends RuleAromatic implements IAlertCounter {
	...
	public SA18() {
		...
		// "[H]C=1C([H])=C([H])C2=C(C=1([H]))C4=C([H])C([H])=C([H])C3=C([H])C(=C(C2=C34)N(=O)O)N(=O)O";
		// //"C=1C=C2C=CC3=CC=CC4=CC=C(C=1)C2=C34";
		/*
		 * examples[0] = ""; examples[1] =
		 * "O=[N+]([O-])C=4C=C2C=CC=C3C=1C=CC=CC=1C(=C23)C=4[N+](=O)[O-]";
		 */
		// "[H]C=1C([H])=C([H])C3=C(C=1([H]))C=2C(=C([H])C([H])=C([H])C=2C3([H])([H]))N([H])C(=O)C([H])([H])[H]";
		// C1=CC=C3C(=C1)CC2=CC=CC=C23";
		/*
		 * C1=CC=C3C(=C1)CC2=CC=CC=C23 C=1C=CC3=C(C=1)CC2=CC=CC=C23
	...
	public String getImplementationDetails() {
		...
		b
				.append("<ol><li>Find essential rings through SSSRFinder.findEssentialRings. This set of rings is uniquely defined.");
		b
				.append("<li>Split into set of fused rings. <li>For each set of fused rings verify for each ring if it is composed of aromatic atoms only.");
		b
				.append("If yes, the ring is considered aromatic and the counter for aromatic rings within this set of fused rings is incremented.");
		b
				.append("<li>If the ring is aromatic, it is verified if it contains an heteroaromatic atoms, and if yes, the counter for heteroaromatic rings is incremented.");
		b
				.append("<li>If the set of fused rings contains > 2 aromatic rings and no heteroaromatic rings, the rule returns true.</ol>");
		return b.toString();
	}
		...
		if (super.verifyRule(mol)) {
			...
			if (nr < 3) {
				...
				for (int i = 0; i < ringsets.size(); i++) {
					...
					if (acceptRingSet(ringset, heteroaromatic_ring_count,
							aromatic_ring_count)) {
