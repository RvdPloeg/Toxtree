 ...
 *         2011
 */
public class Rule21 extends RuleRingMainStrucSubstituents implements
		IAlertCounter {
	...
	public Rule21() throws Exception {
		super();
	...
	public boolean verifyRule(IAtomContainer mol, IAtomContainer selected)
		...
		if ((mainStructure != null)
				&& (!FunctionalGroups.hasGroup(mol, mainStructure, selected))) {
		...
		for (int i = 0; i < rs.getAtomContainerCount(); i++) {
			...
			logger.finer("partitions\t" + s.getAtomContainerCount());
		...
		// throw new
		// DRuleNotImplemented("TODO check for one nitro and up to 3 halogens");
		return true;
	...
	public boolean substituentIsAllowed(IAtomContainer a, int[] place)
			throws DecisionMethodException {
		if (FunctionalGroups.hasMarkedOnlyTheseGroups(a, phenolIDs))
			return true;
		...
		if (FunctionalGroups.hasMarkedOnlyTheseGroups(a, halogenIDs)) {
			...
			if (halogensCount > maxHalogens) {
				logger.finer("Too many halogen substituents\t"
						+ Integer.toString(halogensCount));
				return false;
			} else
				return true;
		} else {
			for (int i = 0; i < a.getAtomCount(); i++)
				if (a.getAtom(i).getSymbol().equals("C"))
					continue;
				else if (a.getAtom(i).getSymbol().equals("H"))
					continue;
				else {
					logger.finer("Forbidden group found\t"
							+ a.getAtom(i).getSymbol());
					return false;
				}
