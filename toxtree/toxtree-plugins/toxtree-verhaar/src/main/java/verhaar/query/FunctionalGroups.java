 ...
 * {@link verhaar.VerhaarScheme}. In future could be modified to use a different
 * approach rather than static methods.
 * 
...
public class FunctionalGroups extends toxTree.query.FunctionalGroups {
	...
	public static QueryAtomContainer ionicGroup() {
		...
		q.setID(IONICGROUP);
		AnyAtom a1 = new ReallyAnyAtom(SilentChemObjectBuilder.getInstance());
		AnyAtom a2 = new ReallyAnyAtom(SilentChemObjectBuilder.getInstance());
		q.addAtom(a1);
		q.addAtom(a2);
	...
	public static QueryAtomContainer ringSubstituted(String substituent) {
		...
		if (substituent == null)
			...
			q.setID(RINGSUBSTITUTED + substituent);
		IAtom a[] = new IAtom[3];
		for (int i = 0; i < a.length; i++) {
			...
			if (i == 1) {
				...
				a[i] = new ReallyAnyAtom(SilentChemObjectBuilder.getInstance());
			q.addAtom(a[i]);
			if (i > 0) {
	 ...
	 * @return
	 */
	public static QueryAtomContainer halogenAtAlphaFromUnsaturation(
			String[] halogens) {
		QueryAtomContainer q = new QueryAtomContainer(
				SilentChemObjectBuilder.getInstance());
		...
		for (int i = 0; i < a.length; i++) {
			if (i == 2) {
				a[i] = new SymbolSetQueryAtom(q.getBuilder());
			...
			default: {
				q.addBond(new OrderQueryBondAromatic((IQueryAtom) a[i],
						(IQueryAtom) a[i - 1], CDKConstants.BONDORDER_SINGLE,
						false, q.getBuilder()));
				break;
			}
			}
	 ...
	 * @return
	 */
	public static QueryAtomContainer halogenAtBetaFromUnsaturation(
			String[] halogens) {
		QueryAtomContainer q = new QueryAtomContainer(
				SilentChemObjectBuilder.getInstance());
	...
	public static QueryAtomContainer halogen(String[] halogens) {
		QueryAtomContainer q = new QueryAtomContainer(
				SilentChemObjectBuilder.getInstance());
		q.setID(HALOGEN);
		SymbolSetQueryAtom h = new SymbolSetQueryAtom(q.getBuilder());
		h.setSymbol("*");
		for (int j = 0; j < halogens.length; j++)
		...
		q.addAtom(h);
		q.addAtom(a);
		q.addBond(new AnyOrderQueryBond((IQueryAtom) a, (IQueryAtom) h,
				CDKConstants.BONDORDER_SINGLE, q.getBuilder()));
		return q;
	...
	public static QueryAtomContainer phenol() throws CDKException {
		...
		for (int i = 0; i < a.length; i++) {
			...
			q.addAtom(a[i]);
			if (i > 0) {
				q.addBond(new verhaar.query.AromaticQueryBond(
						(IQueryAtom) a[i], (IQueryAtom) a[i - 1], q
								.getBuilder()));
			}
		}
		q.addBond(new verhaar.query.AromaticQueryBond((IQueryAtom) a[0],
				(IQueryAtom) a[5], q.getBuilder()));

		SymbolQueryAtom o = new SymbolQueryAtom(MoleculeTools.newAtom(builder,
				"O"));
		q.addAtom(o);
		SymbolQueryAtom h = new SymbolQueryAtom(MoleculeTools.newAtom(builder,
				"H"));
		q.addAtom(h);
		q.addBond(new OrderQueryBond(o, h, CDKConstants.BONDORDER_SINGLE,
				builder));
		q.addBond(new OrderQueryBond(o, (IQueryAtom) a[0],
	...
	public static QueryAtomContainer aniline() {
		...
		SymbolQueryAtom o = new SymbolQueryAtom(new org.openscience.cdk.Atom(
				"N"));
		q.addAtom(o);
		SymbolQueryAtom h = new SymbolQueryAtom(new org.openscience.cdk.Atom(
				"H"));
		q.addAtom(h);
		q.addBond(new OrderQueryBond(o, h, CDKConstants.BONDORDER_SINGLE, q
				.getBuilder()));
		q.addBond(new OrderQueryBond(o, (IQueryAtom) a[0],
				CDKConstants.BONDORDER_SINGLE, q.getBuilder()));
		return q;
	...
	public static QueryAtomContainer pyridine() {
		...
		for (int i = 0; i < a.length; i++) {
			if (i == 0)
				a[i] = new SymbolQueryAtom(new org.openscience.cdk.Atom("N"));
			else
				a[i] = new SymbolQueryAtom(new org.openscience.cdk.Atom("C"));
			q.addAtom(a[i]);
			if (i > 0) {
	...
	public static QueryAtomContainer pyridine_character() {
		...
		for (int i = 0; i < a.length; i++) {
			if (i == 0)
				...
				SymbolSetQueryAtom set = new SymbolSetQueryAtom(q.getBuilder());
				set.addSymbol("C");
				set.addSymbol("N");
				a[i] = set;
			}
	...
	public static QueryAtomContainer ketone_a_b_unsaturated() throws Exception {
		...
		for (int i = 0; i < a.length; i++) {
			...
			case 2: {
				a[i] = new SymbolQueryAtom(new org.openscience.cdk.Atom("C"));
				q.addBond(new OrderQueryBond((IQueryAtom) a[i],
						(IQueryAtom) a[i - 1], CDKConstants.BONDORDER_SINGLE, q
								.getBuilder()));
				break;
			}
			case 3: {
				a[i] = new SymbolQueryAtom(new org.openscience.cdk.Atom("O"));
				q.addBond(new OrderQueryBond((IQueryAtom) a[i],
						(IQueryAtom) a[i - 1], CDKConstants.BONDORDER_DOUBLE, q
								.getBuilder()));
				break;
			}
			case 4: {
				...
				q.addBond(new OrderQueryBond((IQueryAtom) a[i],
						(IQueryAtom) a[2], CDKConstants.BONDORDER_SINGLE, q
								.getBuilder()));
				break;
			}
			}
	...
	public static QueryAtomContainer createAutoQueryContainer(
		...
		for (int i = 0; i < container.getAtomCount(); i++) {
			...
			if (atom instanceof IPseudoAtom)
				...
				queryContainer.addAtom(new SymbolAndChargeQueryAtom(atom));
		}
		...
		for (int i = 0; i < container.getBondCount(); i++) {
			...
			if (bond.getFlag(CDKConstants.ISAROMATIC)) {
				queryContainer.addBond(new verhaar.query.AromaticQueryBond(
				...
				queryContainer.addBond(new OrderQueryBond(
						(IQueryAtom) queryContainer.getAtom(index1),
						(IQueryAtom) queryContainer.getAtom(index2), bond
								.getOrder(), queryContainer.getBuilder()));
			}
		}
	...
	public static QueryAtomContainer cyclicEster() {
		...
		SymbolQueryAtom c = new SymbolQueryAtom(new org.openscience.cdk.Atom(
				"C"));
		SymbolQueryAtom o1 = new SymbolQueryAtom(new org.openscience.cdk.Atom(
				"O"));
		SymbolQueryAtom o2 = new SymbolQueryAtom(new org.openscience.cdk.Atom(
				"O"));
		query.addAtom(r);
		query.addAtom(e);
		query.addAtom(c);
