 ...
 * 
 * @author Nina Jeliazkova <b>Modified</b> 2005-10-31
 */
public class FunctionalGroupstest {
	protected transient static Logger logger = Logger
			.getLogger(FunctionalGroupstest.class.getName());

	public void groupTest(QueryAtomContainer query, String[] smiles,
			boolean[] answers) throws Exception {
		for (int i = 0; i < smiles.length; i++) {
			IAtomContainer mol = FunctionalGroups
					.createAtomContainer(smiles[i]);
	...
	public void allowedGroupTest(QueryAtomContainers query, ArrayList ids,
		...
		for (int i = 0; i < smiles.length; i++) {
			...
			boolean b = FunctionalGroups.hasOnlyTheseGroups(mol, query, ids,
					false);
	...
	public void testHalogenAtBetaFromUnsaturation() throws Exception {
		...
		QueryAtomContainer query = FunctionalGroups
				.halogenAtBetaFromUnsaturation(halogens);
		String[] smiles = { "C1=CC=CC=C1CCl", "c1ccccc1Cl", "c1ccccc1CCl",
				"CC=CCF", "CC=CF", "CC#CCI" };
