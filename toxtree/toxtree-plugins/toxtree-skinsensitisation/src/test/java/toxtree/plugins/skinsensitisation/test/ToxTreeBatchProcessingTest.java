...
public class ToxTreeBatchProcessingTest {
	protected static Logger logger = Logger
			.getLogger(ToxTreeBatchProcessing.class.getName());
	protected ToxTreeBatchProcessing batch = null;
	protected String config = "data/batch.cfg";
	protected String configInterrupted = "data/batchInterrupted.cfg";
	protected IDecisionMethod rules;

	@Before
	public void setUp() throws Exception {
		Introspection.setLoader(getClass().getClassLoader());
		rules = new SkinSensitisationPlugin();
		ToxTreeBatchProcessing batch = null;
		try {
			URL url1 = getClass().getClassLoader().getResource("data/test.sdf");
			Assert.assertNotNull(url1);
			Assert.assertTrue(new File(url1.getFile()).exists());
			batch = new ToxTreeBatchProcessing(url1.getFile(),
					"batchResults.sdf");
	...
	public void testSuccessfullBatch() throws Exception {
		...
		Assert.assertNotNull(url1);
		Assert.assertTrue(new File(url1.getFile()).exists());
		ToxTreeBatchProcessing bp = new ToxTreeBatchProcessing(url1.getFile(),
				"batchResults.sdf");
		...
		bp.start();
		Assert.assertFalse(bp.getConfigFile().exists()); // tmp files deleted on
															// success
	...
	/*
	 * public void testBatchInThread() { try { ToxTreeBatchProcessing bp = new
	 ...
	 * bp.setDecisionMethod(new CramerRules()); Thread thread = new Thread(bp);
	 * thread.run();
	 * 
	 ...
	 * 
	 * } catch (BatchProcessingException x) { x.printStackTrace(); fail(); } }
	 */
	@Test
	public void testBatchInThread() throws Exception {
		...
		Assert.assertTrue(new File(url1.getFile()).exists());
		ToxTreeBatchProcessing bp = new ToxTreeBatchProcessing(url1.getFile(),
		// "cellbox:/nina/Databases/ligand_info_subset_1_dos.sdf", space after
		// property line
	...
	public void testInterruptedJob() throws Exception {
		...
		batch.setDecisionMethod(new SkinSensitisationPlugin());
		...
		/*
		 * // bp.setDecisionMethod(new CramerRules()); bp.addObserver(new
		 * Observer() {
		 * 
		 * public void update(Observable o, Object arg) { if (o instanceof
		 * BatchProcessing) { BatchProcessing b = (BatchProcessing) o;
		 * System.out.println(b); if (b.getReadRecordsCount()> 11) try {
		 * b.deleteObservers(); b.pause(); System.out.println(b); } catch
		 * (BatchProcessingException x) { x.printStackTrace(); fail(); }
		 * 
		 * }
		 * 
		 * } }); bp.start();
		 ...
		 * 
		 * bp.addObserver(new Observer() { public void update(Observable o,
		 * Object arg) { System.out.println("Continued ...\t" + o);
		 * 
		 * } } ); bp.start();
		 */
