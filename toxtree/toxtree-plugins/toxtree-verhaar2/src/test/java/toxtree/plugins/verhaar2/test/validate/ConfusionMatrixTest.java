...
public class ConfusionMatrixTest {
	...
	protected transient static Logger logger = Logger
			.getLogger(ConfusionMatrixTest.class.getName());
	...
	public void testVerhaar2() throws Exception {
		...
		testFile(
				getClass()
						.getClassLoader()
						.getResource("toxtree/plugins/verhaar2/Verhaar2000.sdf")
						.getFile(),
	...
	public void testVerhaar2Enoch2008() throws Exception {
		...
		testFile(
				getClass().getClassLoader()
						.getResource("toxtree/plugins/verhaar2/enoch08.csv")
						.getFile(), new String[] { "Expected Verhaar scheme",
						"Verhaar_1_60", "MOA" });
	}
	...
	public void testVerhaar2EPAFHM() throws Exception {
		...
		testFile(
				getClass()
						.getClassLoader()
						.getResource(
								"toxtree/plugins/verhaar2/EPAFHM_v4b_617_15Feb2008.sdf")
						.getFile(), new String[] { "MOA" });
	}

	protected void testFile(String filename, String[] expectedColumns)
			throws Exception {

		Assert.assertTrue(filename, new File(filename).exists());
		IteratingSDFReader reader = new IteratingSDFReader(new FileInputStream(
				new File(filename)), SilentChemObjectBuilder.getInstance());
				...
				try {
					...
					for (int i = 0; i < expectedColumns.length; i++) {
						...
						if (expectedValues[i] == null)
							continue;
						if ("".equals(expectedValues[i]))
							continue;
						cmatrix.addEntry(expectedValues[i].toString(), result
								.getCategory().getName());
					}
					ok++;
