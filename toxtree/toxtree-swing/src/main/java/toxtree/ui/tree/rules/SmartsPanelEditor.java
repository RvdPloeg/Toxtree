...
public class SmartsPanelEditor extends JPanel {
	...
	public void setSmartsPatternFactory(
			ISmartsPatternFactory smartsPatternFactory) {
	...
	public SmartsPanelEditor(String name, String smarts,
			ISmartsPatternFactory smartsPatternFactory) {
		...
		try {
			...
			testMolecule = p.parseSmiles("CCCCCC");
			//panel2D.setAtomContainer(testMolecule, true);
			AtomContainerManipulator
					.percieveAtomTypesAndConfigureAtoms(testMolecule);
		} catch (Exception x) {
		}
		try {
			smartsPattern = getSmartsPatternFactory().createSmartsPattern(
					smarts, false);
		...
		createPopupmenu();
		JScrollPane scrollPane = new JScrollPane(pattern,
		...
		// c.add(new
		// JScrollPane(status,JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS));
	...
	public void isValidSMARTS(String smarts) throws Exception {
		if (smartsPattern == null)
			smartsPattern = smartsPatternFactory.createSmartsPattern(smarts,
					false);
		else
