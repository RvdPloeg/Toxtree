...
public class DataContainer extends Observable {
	...
	public DataContainer(File inputFile) {
		super();
		if (logger == null)
			logger = Logger.getLogger(DecisionMethodData.class.getName());
	...
	public void clearFilters() throws FilterException {
		if (containers instanceof FilteredMoleculesIterator) {
			...
			throw new FilterException("Filtering not supported!");
	}

	public void selectFilter(Component parentComponent) throws FilterException {
		if (containers instanceof FilteredMoleculesIterator) {
			((FilteredMoleculesIterator) containers)
					.selectFilter(parentComponent);
			setChanged();
			notifyObservers();
	...
	public void filter(IDecisionResult treeResult) throws FilterException {
		if (treeResult == null)
			throw new FilterException("Filter not defined!");
		IDecisionCategories categories = treeResult.getDecisionMethod()
				.getCategories();
		if (categories == null)
			throw new FilterException("Categories not defined!");
		if (containers instanceof FilteredMoleculesIterator) {
			/*
			 * for (int i=0; i < categories.size();i++) {
			 * ((FilteredMoleculesIterator) containers).addFilter( new
			 * CategoryFilter(treeResult.getResultPropertyName() ,
			 * categories.get(i)) ); }
			 */
	...
	public int lookup(String field, Object value, boolean silent) {
		...
		for (int record = 0; record < containers.getMoleculesCount(); record++) {
			...
			try {
				...
				if (ac.getProperty(field) == null)
					continue;
				String property = ac.getProperty(field).toString()
						.toLowerCase();
				...
				logger.log(Level.WARNING, "Error when processing record %s %s",
						new Object[] { record, x });
				x.printStackTrace();
	...
	public void openFile(File input) {
		...
		try {
			...
			if ((x.getCause() != null) && (x.getCause().getMessage() != null))
				...
				msg = "Error";
			ToxTreeActions
					.showMsg(x.getMessage() + "\n" + x.getFilename(), msg);
		}
		setChanged();
		notifyObservers();
		/*
		 * 
		 * return null; } //Runs on the event-dispatching thread. public void
		 * finished() { first(); } };
		 * 
		 * worker.start();
		 */
	...
	public void saveFile(File output) {
		...
		/*
		 * return null; } //Runs on the event-dispatching thread. public void
		 * finished() { first(); //setChanged(); redundant - first() already
		 * called these //notifyObservers(); } }; worker.start();
		 */
	}
